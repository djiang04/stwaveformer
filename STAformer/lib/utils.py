...
def PCA_decomposition(A,d):
  C = A - np.mean(A, axis=0)
  V = np.cov(C)
  _, vectors = np.linalg.eigh(V)
  return vectors[:,:d]
...
def PCAd(x):
    ...
    for i in range(S):
        for j in range(N):
            L = 10
            K = T - L+1
            X = np.column_stack([x[i,k:k+L, j] for k in range(K)]) 
            d=2
            vectors = PCA_decomposition(X, d)
            X_elem_pca = np.array([np.dot(np.outer(vectors[:,i],vectors[:,i].T), X) for i in range(d)])
            xl[i,:, j] = X_to_TS(X_elem_pca[0]).reshape(-1,1)
            xh[i,:, j] = X_to_TS(X_elem_pca[1]).reshape(-1,1)
    return xl, xh
